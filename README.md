...
菜单流程是 `查看状态 / 设置界面语言 / 清除 / 退出`;选择 `设置界面语言` 后,再用上下键选择 `简体中文 / English / 返回`。
...
`/ui-language` 写入 `~/.pi/agent/settings.json` 的 `uiLanguage` 字段,默认是简体中文。当前支持 `zh-CN` 和 `en-US`;UGK 自有菜单、状态面板、header/footer、CDP/MCP/doctor/plan 常用提示会随它切换。
