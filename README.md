...
`/ui-language` 写入 `~/.pi/agent/settings.json` 的 `uiLanguage` 字段,默认是简体中文。当前支持:简体中文、English、日本語、한국어、Français、Deutsch、Español、Português、Русский。UGK 自有菜单、状态面板、header/footer、CDP/MCP/doctor/plan 常用提示会随它切换;未补字典的动态长句会退回 English。
