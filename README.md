...
```text
✨ 发现可用更新! 1.0.0 (aaaaaaa) -> bbbbbbb

发布记录: https://github.com/mhgd3250905/ugk-tui/commits/main

› 1. 立即更新(运行 `git pull --rebase origin main && npm install`)
  2. 跳过本次
  3. 跳过直到下个版本
...
| `web_search` / `web_read` | 隔离 headless Chrome 公网搜索和网页正文读取(Google 优先,Bing 中国版兜底) |
...
| `/web-search` | 管理隔离的 web_search Chrome(status/open/restart) |
...
`web_search` 和 `web_read` 使用独立 Chrome profile(`~/.ugk/web-search-profile`)和端口 9223,不共享本地登录态 Chrome。普通公网检索优先用它,需要 SSO/cookie/CAPTCHA 时再用 `chrome_cdp`。
...
旧的 `/doctor` 表格已改为引导式入口。遇到本机环境问题时,可用 `/doctor`,也可以直接对 agent 说:
