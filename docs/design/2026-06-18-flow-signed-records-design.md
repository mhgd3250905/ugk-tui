...
状态:部分实现(task.json 签名链已落地;review/validation 验签 + 迁移重签为已知 TODO)
...
#### 约束二:反馈是引导,不能瞎反馈
**失败反馈 = 给 agent 的指令。瞎反馈会让 agent 在错误方向狂奔,把局面越弄越糟。** 反馈必须满足:

1. **恢复动作只有一个,且明确**:不给 agent 多个选项让它猜。就一个动作。
2. **恢复动作必须安全**:不能造成新损害。绝不能是"删数据"、"改状态记录"、"手动修复 task.json"——agent 修不了(没密钥),瞎修只会更乱。
3. **回到稳定态,而非修复损坏态**:签名失败的记录不可信 → 不试图修它 → 让 task 回到一个已知安全的状态(prove 失败回 draft;review 损坏回到需重新 review)。判定记录损坏时,agent 不当法官。
4. **决策权交回用户**:严重损坏时,反馈引导 agent **告知用户**("这个 task 的记录有问题,建议重新 prove"),而非 agent 自主处置。agent 是执行者,判定记录异常时法官是用户。

#### 具体反馈映射(实现时照此)

| 损坏场景 | 正确反馈(安全引导) |
|---|---|
| task.json 验签失败 | "task `x` 的状态记录不可用,建议从证明阶段重新开始(`/flow task prove x`)" |
| review.json 验签失败 | "task `x` 的复盘记录不可用;请告诉用户这次 run 需要重新复盘" |
| validation.json 验签失败 | "run `x` 的校验记录不可用,runtime 将重新校验产出" |

每条都符合:不提签名、单一动作、动作安全、严重时交回用户。

这条是安全设计的一部分,实现时必须逐条对照检查,review 时也要专门验证"反馈里有没有泄露签名机制的字眼"以及"反馈动作是否安全"。
...
## 十四、实现状态 + 已知 TODO(独立 review 后更新)

### 已落地(task.json 完整签名链)
- flow-signing.ts:密钥派生(HKDF)、HMAC、canonicalJSON、验签、CORRUPT_FEEDBACK。
- task-store.ts:writeFlowTask 签名 + 关闭迁移窗口;readFlowTask 验签 + `_signatureBroken` 标记。
- lifecycle-gates.ts:readTaskMetadata 验签,损坏返回中性 CORRUPT_FEEDBACK。
- task-state.ts:transition 拒绝 `_signatureBroken` 记录。
- review-store.ts / run-validation.ts:写时签名(start/accept/reject/validateFlowRun 带 cwd)。
- 反馈安全:CORRUPT_FEEDBACK 文案测试扫描禁词(签名/密钥/HMAC 等零出现)。
- undefined 字段签名 bug 已修(extractCovered 统一 undefined→null)。

### 已知 TODO(独立 review 发现,本次未做)

**P1:review.json / validation.json 读取层未验签**
- 签名写入了(start/accept/reject/validateFlowRun 写时签),但 readFlowReview / readFlowRunValidation **不验签**。
- 影响:agent 仍能改 review.json 的 status(rejected→accepted)不被发现——签名链对 review/validation 只写不验,防不住。
- 未做的原因:改 readFlowReview/readFlowRunValidation 签名(加 cwd)影响所有调用点 + 测试造数据方式,当前会话内做不稳。需单独一轮:加 Verified 变体 + 决策点接入 + 测试全面更新。
...
- 跨机器:.migrated 跟着拷贝,旧 _sig 验不过且窗口已关 → 卡死,无重签路径。
- 未做的原因:需要"启动期扫描重签"或 reset-signing 命令(设计第五/七节),是较大改动。

**P3-4:canonicalJSON 嵌套对象 undefined 未测**
- 当前 covered 字段全是标量,extractCovered 顶层归一已够。但若未来 covered 扩展到对象字段,嵌套 undefined 会不一致。建议补测试锁契约。
