...
| **executing** | main **亲手**把任务做一遍 | **同一个 context 继续**(保留决策链) | read/write/edit/bash,**禁止 subagent** |
...
|---|---|---|---|---|
| **task-creator**(`/task` planning+executing) | 创造阶段全程 | 长 context(plan+execute 共享) | Spec + 完整执行决策链 | read/write/edit/bash,**禁止 subagent** |
| **reviewer**(`/task` reviewing) | 复盘阶段 | 短 context(注入执行摘要) | 执行摘要 + Spec,不看原始 transcript | questionnaire + read |
| **worker**(复用时) | 每次 `/task run` 派出 | 短 context(注入 skill) | skill + contract,**不看 verify** | 按 agent 定义,通常 read/write/edit/bash |
...
| `/task edit <name>` | 编辑模式 | 加载现有 taskbook,用 questionnaire 重新对齐(类似 `/judge edit`) |
...
- **PASS**:artifact 绝对路径 + 文件大小 + (.json 文件)内容 + `verify: 全过` + worker 摘要
- **FAIL**:失败断言(含 expected/actual)+ worker 摘要

不再只显示一个 PASS 标记。

### `/task run` 进度 widget(dogfood 后新增)

`setTaskRunWidget`(`task.ts:317`)在 worker/verify 阶段用 `ctx.ui.setWidget("task-run-view", ...)` 显示当前状态(尝试 N/3、worker 中/verify 中),run 结束清理。
