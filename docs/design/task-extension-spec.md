...
### verify 设计先于脚本书写(dogfood 后新增)

review/update/repair 阶段在输出 JSON 前必须先用 questionnaire 确认 verify 设计,包括 artifacts、assertions、failure cases、runtime input、allowed variability。确认后才写 `verify.mjs`,避免 agent 先脑补脚本再让用户被动接受。
