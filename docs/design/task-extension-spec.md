...
|---|---|---|---|
| **planning** | 跟人对齐目标、约束、验收 | 长 context(后续 execute 共享) | questionnaire + Spec 解析 |
...
**直接复用 Judge 的 RequirementsSpec,零改动**:
...
直接复用 `extractRequirementsSpec`/`normalizeSpec`/`formatRequirementsSpec`(`extensions/judge/judge-utils.ts`)。
...
|---|---|---|---|---|
| **task-creator**(`/task` planning+executing) | 创造阶段全程 | 长 context(plan+execute 共享) | Spec + 完整执行决策链 | planning 只读;executing 继承 active tools,只排除 subagent |
...
| `extractRequirementsSpec` | `extensions/judge/judge-utils.ts:234` | 从 LLM 输出捞 Spec |
| `normalizeSpec` | `extensions/judge/judge-utils.ts:208` | Spec 校验(strict 版) |
| `formatRequirementsSpec` | `extensions/judge/judge-utils.ts:254` | Spec 格式化展示 |
| `isRequirementsSpec`(lax 版) | `extensions/judge/taskbook.ts:38` | 加载 spec.json 时用 |
...
### 7.1 交互范式(硬约束,照搬 `/cdp` 和 `/judge`)

`/task` 的交互**必须严格复刻** `/cdp` 和 `/judge` 的现成模板,这是用户明确要求的。具体规则:
...
- **task-prompts.ts**:Spec 解析复用 Judge 的 `extractRequirementsSpec`
...
`/task run` 最终 FAIL 后进入 `pendingTransition: "repair"`。用户可按 Enter/输入意见进入修正复盘,或在 `/task` 菜单选择"修正本 taskbook / 重新运行 / 查看 taskbook 详情 / 放弃"。修正复用 reviewing/save 流程,上下文包含失败断言、worker 摘要、旧 skill/verify/contract,不引入 plan mode 依赖。

### `/task edit` 是更新已有 taskbook(dogfood 后新增)

`/task edit <name>` 不再进入 planning/executing 从头创建。它加载现有 `spec.json`、`skill.md`、`verify.mjs`、`contract.json`,直接进入 reviewing,并要求 reviewer 基于旧内容做最小增量修改。保存默认回写原 taskbook scope,保留 `runs[]` 历史;因为没有本次 execute outputDir,更新保存不会做首次产物自证,而是提示用户重新 `/task run` 验证。

### skill/verify 设计先于文件书写(dogfood 后新增)

review/update/repair 阶段在输出 JSON 前必须先用 questionnaire 确认两类设计。先确认 `skill.md` 的可复用执行路径,包括 source/method、required steps、noise to omit、output path and format;再确认 `verify.mjs` 的验收设计,包括 artifacts、assertions、failure cases、runtime input、allowed variability。确认后才写文件内容,避免 agent 只盯 verify 或先脑补脚本再让用户被动接受。

如果 worker 复用路径需要受保护工具,review 产出的 `contract.json` 应写 `requiredTools`,例如 `["chrome_cdp", "alpha__echo"]`。老 taskbook 没有该字段时,`/task run` 仍会从 `skill.md`/`contract.json` 文本里识别当前 active tools 中被明确提到的受保护工具。
