...
> **状态**:迁移完成。存量 task 的开发期测试已随包进入 `tests/` 子目录,仓库根旧 fixture 已清理。
...
runner 按 `--task=<name>` 解析已安装 task 包(user scope 优先 → project scope 兜底),从包内读 contract/skill 和 `tests/eval.cases.json`。报告输出随包走(`<taskDir>/tests/eval.report.{json,md}`)。
...
- 存量 task 的 `eval.cases.json` 已迁入各自包内 `tests/` 子目录。
- 有 `verify.mjs` 的糖尿病 task 已补 `verify.test.mjs`。
- 有本地自检入口的脚本已补脚本冒烟测试;无自检入口的联网 collector 不做假冒烟。
- `tests/fixtures/taskbooks/` 和 `tests/fixtures/dispatcher-evals/` 已删除。
- `scripts/eval-dispatcher.mjs` 已删除迁移期 `--legacy-fixtures` 回退逻辑。
