...
1. **OAuth state 未与 challenge 绑定**(审查 C1):state 防 OAuth CSRF,但未与 cli challenge 绑定。理论重放面需 XSS/中间人配合。**修复**:start 生成 state 存 pending,callback 校验 state∈pending(challenge)。
2. **challenge cookie 无 HttpOnly**(审查 C2):`cli-auth` 页面 JS 写 cookie 无法设 HttpOnly。challenge 是 5min 单次握手标识(cli_token 从不进 cookie)。**修复**:改 `startCliAuth` 服务端 `Set-Cookie`(HttpOnly)+ 302,删前端写 cookie(改动较大)。
3. **`/api/cli/auth/start` 无速率限制**(审查 M3):公开端点可灌 pending 表。**修复**:Cloudflare WAF/Turnstile,或服务端生成 challenge。
4. **cli_tokens 表无自动过期**(审查 H2 余项):token 长期有效,仅手动 DELETE 吊销。**修复**:加 `created_at` 过期清理或吊销接口。
