...
- **18 个 commit**(不含 UI 闪烁修复 e050137,已移出本 PR),每个都是通过完整测试的独立小步,可单独 review。
...
分三阶段(A 清理 / B 中心化 / C 语义收口与安全),共 18 个 commit:
...
git log --oneline main..HEAD   # 18 个 commit
```
...
不需要逐行看 diff——18 个 commit 每个都有清晰的 commit message 说明意图。
...
**修复**:`git rebase --onto main e050137` 将 e050137 移出分支(它仍保留在 `codex/flow-ui-flicker-investigation` 分支不受影响)。移出后 PR 全部 commit 都属于状态机中心化主题。当前 18 个 commit = 16 改动 + 1 PR 报告 + 1 审核反馈修复。
