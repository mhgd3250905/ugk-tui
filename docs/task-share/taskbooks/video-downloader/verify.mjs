...
function requestedSubtitleLanguages(languages, subLangs = "all") {
	const raw = String(subLangs || "all").trim();
	if (!raw || raw === "all") return languages;
	const rules = raw.split(",").map((item) => item.trim()).filter((item) => item && !item.startsWith("-"));
	return languages.filter((language) => rules.some((rule) => {
		if (rule === language) return true;
		if (rule.endsWith(".*") && language.startsWith(rule.slice(0, -2))) return true;
...
if (!outputDir || !existsSync(outputDir)) {
	...
	const requestedLanguages = requestedSubtitleLanguages(subtitleLanguages, summary?.subLangs);
	if (requestedLanguages.length > 0 && subtitles.length === 0) {
		fail("subtitles downloaded when available", requestedLanguages.join(", "), "no subtitle files");
	}
	if (summary) {
		...
		if (requestedLanguages.length > 0 && (!Array.isArray(summary.subtitleFiles) || summary.subtitleFiles.length === 0)) {
