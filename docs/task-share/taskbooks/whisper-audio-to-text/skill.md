...
不要绕过脚本直接调用 `whisper` 或 `whisper --help`;脚本会给 Whisper 子进程设置 UTF-8 环境变量,避免 Windows GBK 输出导致 argparse traceback。

## 产出

- `transcript.txt`: 纯文本转写。
- `transcript.srt`: SRT 字幕,可传给 `subtitle-fluent-translator`。
