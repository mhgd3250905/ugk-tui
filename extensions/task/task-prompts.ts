...
2. SKILL DESIGN GATE: before writing skill.md, you MUST use questionnaire to confirm the reusable worker path with the user. The questions must cover source/method, required steps, noise to omit, and output path and format. Present your proposed defaults from the successful run; do not ask vague "is this okay?" questions. If the capability already exists as an installed skill, prefer having the taskbook call that skill (worker subprocesses load installed skills) rather than duplicating its logic; declare any protected tools the called skill needs (e.g. chrome_cdp, MCP tools) in contract.requiredTools so the authorization gate and CDP tab isolation take effect.
...
4. Draft contract.json: the shared worker/verify/checker contract with outputDir, artifacts, runtimeInput, optional runtimeInputMeta defaults/descriptions, and requiredTools when the worker path needs protected tools such as chrome_cdp or MCP tools like server__tool. If the worker shells out to external CLIs (e.g. yt-dlp, ffmpeg, pandoc, python, curl), declare them in contract.requiredBinaries so the preflight check fails fast with an install hint instead of crashing mid-run; requiredBinaries makes the task portable (machines missing the CLI get told what to install). For expensive/non-repeatable collectors, set maxRetry: 0 so verify failure does not rerun the worker. If the user specified a fixed final output directory, put that absolute path in contract.outputDir; otherwise use the runtime default. Every runtimeInputMeta key MUST also appear in runtimeInput.
