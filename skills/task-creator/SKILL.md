...
> **API key / 环境变量类依赖**：声明在 `contract.requiredEnv`（如 `"requiredEnv": ["MIMO_API_KEY"]`）。机制在 worker spawn 前会 hydrate（Windows 从用户环境补全）并检查存在性，缺失则直接报错并提示 `setx` 配置——**worker 进程内不重复检查 key 是否存在**（同 CDP 的分层原则：声明层管就绪，worker 只管用）。范本：`subtitle-to-speech/contract.json` 的 `requiredEnv: ["MIMO_API_KEY"]`。如果你封装的是外部 API（语音/TTS/翻译/LLM），几乎一定用这个字段，别在脚本里自己 `if (!apiKey) throw`。
...
```
> - `outputDir` 固定写 `"<runtime>"`，由系统在运行时注入实际输出目录。worker 进程和 verify.mjs 通过环境变量 **`$TASK_OUTPUT_DIR`** 拿到这个实际目录——contract 里写 `<runtime>` 是占位符，脚本/verify 里用 `process.env.TASK_OUTPUT_DIR` 读真实值。
...
4. **测试迭代循环（核心，别跳过）** — 五件套写到 scope 目录后，**不要急着 `/task save` 确认交付**，先在写好的目录里跑一轮"边界刁难→优化→再跑"的迭代循环（五件套可以边写边改，迭代期间它们就在 scope 目录里，这是正常的——"落盘"指的是 `/task save` 确认达标，不是写文件）。这一步把过去"实际使用才暴露"的问题提前到创建期抓出来。详见下方「测试迭代循环」章节——**这步做透了，第 5 步一次 PASS，不用反复试错**。
5. **落盘 + 试跑** — 四条收敛标准全满足（已收敛状态）后，`/task save` 确认交付，再 `/task run <name> <自然语言输入>` 试一次真实场景看 PASS。如果第 4 条（worker 真跑）不满足，只能交付"待验证"状态并明确标注，不能声称已收敛。
...
> **底线原则（最重要，先记住）**：测试分两层，**不能互相替代**：
> 1. **verify 判别力**（构造产物喂 verify）——验证"判官靠不靠谱"
> 2. **worker 真跑**（真实 API key/模型跑脚本）——验证"脚本本身能不能跑通"
>
> 调外部 API 的 task，鉴权头错、endpoint 错、参数格式错——这些 verify 测不出，只有真跑 worker 才暴露。**绝不能因为"verify 判别力全过"就声称 worker 已验证**。详见 4a 第二层。

### 4a. 真跑自证（happy path 必须过）—— 注意"真跑"有两层

这里有个关键区分：**"验证 verify 判得对"和"验证 worker 脚本本身能跑通"是两回事，不能互相替代。**

**第一层：verify 判别力自证（构造产物 → 喂给 verify）**
...
连 happy path 都不过，说明 verify 写错了或 skill 指令有歧义——先修这个，别往下走。这一步等价于 saveCurrentTask 的"正例自证"，你提前做能省一次被打回。**这一层验证的是"verify 这个判官靠不靠谱"，用任何产物（execute 真实的、或你构造的）都能测。**

**第二层：worker 脚本真跑（验证脚本本身能跑通）—— 调外部 API 的 task 必做**

如果你的 task 调外部 API（语音/TTS/翻译/LLM）、用本地模型、依赖登录态，**必须真的跑一次 worker 脚本**（不只是把产物喂给 verify）。理由：构造产物测不出这些 bug——

- **鉴权错**（如 MiMo API 要求"两种认证方式选其一，不能同时用"，脚本同时带了两个 header → 401）
- **endpoint 路径错**（API 路径拼错）
- **参数格式错**（请求体结构、音频编码方式不对）
- **响应解析错**（API 返回结构和脚本假设的不一样）

这些只有真跑 worker 才暴露。verify 哪怕判别力满分（100% 边界都判对），也抓不到——因为 verify 拿到的是"假设正确的产物"，它不知道产物是怎么来的。

**所以：调外部 API 的 task，worker 脚本必须真跑至少一次**（设置好 API key / 模型路径，跑一个真实输入，确认产出正确）。这一层用构造产物**无法替代**。

**如果创建者当下没有运行资源（没 API key / 没装模型 / 没登录态）怎么办？**

不能假装跑过了。两个选择：
1. **找有资源的环境真跑一次**（让用户提供 key、或换台装了模型的机器）
2. **诚实标注"worker 未真跑验证"**，把 taskbook 标为"待验证"状态——骨架和 verify 判别力可以先做好，但必须在交付报告里**明确说明 worker 脚本未经真跑、可能存在鉴权/endpoint/参数类 bug**，等有资源时补测。

**绝不能**因为"构造产物 verify 都判对"就声称 worker 已验证、task 已收敛。这是最严重的概念混淆——它会让 task 看起来达标、实际一跑就炸（鉴权 401、endpoint 404），用户第一次用就踩坑，恰好是这套 SOP 要消灭的情况。
...
### 4d. 收敛判定（四条全满足才能声称"已收敛"）

完成 4a-4c 后，逐条核对。**"已收敛"和"待验证"是两个状态，别混**：

1. **verify 判别力真跑 PASS**：execute 真实产物跑 verify，exit=0。（这是验证 verify 靠不靠谱）
2. **边界全覆盖**：空/坏/截断三类产物，verify 判得全对（该 PASS 的 PASS、该 FAIL 的 FAIL 且指出问题）。
3. **暴露的问题已优化**：4b 发现的每个假通过/漏判，都已修进 verify 或脚本，且重跑确认修好。
4. **worker 真跑已验证**（调外部 API/模型/登录态的 task 必须满足）：worker 脚本用真实资源（API key/模型/登录态）跑过至少一次，产出正确。**没真跑 = 这条不满足。**

四条全满足 → **已收敛**，可以进第 5 步落盘。

第 4 条不满足（没资源真跑 worker）→ **待验证状态**。这种情况 taskbook 骨架可以交付（verify 判别力、纯函数、preflight 都能先做好），但**必须**：
- 在交付报告里明确写"worker 脚本未经真跑验证，可能存在鉴权/endpoint/参数类 bug，待有资源时补测"
- **不能**声称"已收敛/已达标/测试通过"——第 1-3 条满足只代表 verify 可靠，不代表 worker 能跑通
- 等有资源时第一时间补 worker 真跑，把状态从"待验证"转成"已收敛"

**绝不能**用"verify 判别力测试全过"来暗示"worker 已验证"——这是概念混淆，会让 task 看起来达标、实际一跑就炸（用户第一次用就踩坑，恰好是这套 SOP 要消灭的）。
...
> **eval 框架在哪、怎么跑**：`scripts/eval-dispatcher.mjs` 和 `tests/fixtures/` 是 **ugk-core 仓库** 的设施（不在 taskbook 目录里）。如果你在开发 ugk-core（有仓库 clone），直接 `npm run eval:dispatcher -- --task=<你的task>`。如果你是**独立的 taskbook 作者**（只有 `~/.pi/agent/tasks/`，没有仓库），eval 框架跑不了——这时最现实的 dispatcher 测试是**直接 `/task run` 真跑**（步骤 5）：dispatcher 在真实 run 里就被调用，翻错会直接导致产物错或 FAIL。用几种不同表述跑（明确的、模糊的、缺必填的），观察 runtimeInput 翻译对不对，这是 eval 的轻量替代。
