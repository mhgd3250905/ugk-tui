...
test("chooseTaskbookName ignores the legacy dedicated tag", async () => {
	const { pi, commands } = makePi();
	...
	try {
		...
		assert.deepEqual(chooseSel!.options, ["ada", "zoo"]);
		const detailSel = selections.find((s) => s.title.startsWith("taskbook:"));
		assert.ok(detailSel, "应进入 taskbook 详情");
...
test("task details do not offer dedicated controls", async () => {
	const { pi, commands } = makePi();
	const { cwd, ctx, selections } = makeCtx();
	registerTask(pi as any);
	mockTaskGuideRunner("1. 任务目标: x\n5. 产物契约: 未声明");
	try {
		...
		await commands.get("task").handler("show", ctx);
		const detail = selections.find((selection) => selection.title === "taskbook: ada");
		assert.deepEqual(detail?.options, ["task 导览", "task 编辑", "返回", "Exit"]);
	} finally {
		setTaskGuideRunnerForTests(undefined);
		rmSync(cwd, { recursive: true, force: true });
	}
