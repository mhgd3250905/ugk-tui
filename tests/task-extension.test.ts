...
test("详情页翻转专用后菜单刷新文案且不退出,可连续翻转", async () => {
	const { pi, commands } = makePi();
	const { cwd, ctx, selections } = makeCtx();
	registerTask(pi as any);
	// ponytail: 用序列控制 select —— 设专用 → 取消专用 → Exit,验证翻转后菜单不退出且文案变化
	let detailSelectCount = 0;
	ctx.ui.select = (title: string, options: string[]) => {
		selections.push({ title, options });
		if (title === "选择 taskbook") return options[0];
		// 详情菜单: 三次进入,分别选 设专用/取消专用/Exit
		detailSelectCount += 1;
		if (detailSelectCount === 1) return options.find((o) => o.startsWith("设为专用"))!;
		if (detailSelectCount === 2) return options.find((o) => o.startsWith("取消专用"))!;
		return "Exit";
	};
	try {
		// ada 字典序在前,被选中。初始无 dedicated tag。
		await saveTaskbook("project", cwd, "ada", {
			description: "翻转测试",
			spec, skill: "# x\n", verify: "process.exit(0);\n",
			contract: { artifacts: [] },
		...
		await commands.get("task").handler("show", ctx);
		// 1. 详情菜单被调用了 3 次(设/取消/Exit),证明翻转后没退出
		const detailSels = selections.filter((s) => s.title.startsWith("taskbook:"));
		assert.equal(detailSels.length, 3, `详情菜单应出现 3 次,实际 ${detailSels.length}`);
		// 2. 第一次菜单含"设为专用(当前🔓)"(初始非专用)
		assert.ok(detailSels[0].options.some((o) => o.startsWith("设为专用(当前🔓)")), "初次应显示设为专用");
		// 3. 第二次菜单含"取消专用(当前🔒)"(已设为专用,文案刷新)
		assert.ok(detailSels[1].options.some((o) => o.startsWith("取消专用(当前🔒)")), "翻转后应显示取消专用");
		// 4. 最终 tag 回到无 dedicated(设了又取消)
		const final = await loadTaskbook(cwd, "ada");
		assert.equal(final!.taskbook.tags?.includes("dedicated") ?? false, false, "最终应无 dedicated");
	} finally {
		rmSync(cwd, { recursive: true, force: true });
	}
});
